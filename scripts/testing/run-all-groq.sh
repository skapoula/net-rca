#!/usr/bin/env bash
# run-all-groq.sh — execute all test phases in order using Groq inference (Groq variant)
# Phases 1-4 are identical to the local-pod variant; only pre-flight and startup differ.
set -euo pipefail
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Create a single timestamped results directory shared across all phases
export RESULTS_DIR="/workspace/net-rca/test-results/$(date +%Y%m%d-%H%M%S)-groq"
mkdir -p "$RESULTS_DIR"
echo "Results directory: $RESULTS_DIR"

source "$SCRIPT_DIR/helpers.sh"

log "Starting full test run (Groq variant). Results: $RESULTS_DIR"

"$SCRIPT_DIR/phase0-preflight-groq.sh"  || { echo "GATE: Phase 0 failed — aborting"; exit 1; }
"$SCRIPT_DIR/phase05-start-groq.sh"     || { echo "GATE: Phase 0.5 failed — aborting"; exit 1; }
